...
int ImgDataTEST::runUnits() {
    ...
    logger->trace("==============================");
	if (res != 0)
        logger->warn(std::to_string(res)+" warning(s) in unit tests");
...
int ImgDataTEST::T_getHeight() {
    ...
    logger->trace("    "+std::to_string(data->getHeight()));
...
int ImgDataTEST::T_getWidth() {
    ...
    logger->trace("    "+std::to_string(data->getWidth()));
	if (data->getWidth() == 2)
        logger->trace("    ok");
	else {
...
int ImgDataTEST::T_setImg() {
    ...
    logger->trace("    Obj 1 refcount: "+std::to_string(t1rc));
    logger->trace("    Obj 2 refcount: "+std::to_string(t2rc));
    if (t1rc+t2rc==2)
        logger->trace("    ok");
    else {
    ...
    imgrc = testImg->img.u->refcount;
    logger->trace("    Obj 1 refcount: "+std::to_string(t1rc));
    logger->trace("    Obj 2 refcount: "+std::to_string(imgrc));
    if (t1rc+imgrc==2)
        logger->trace("    ok");
    else {
        logger->warn("    NOT ok, refcount should be 1 for both");
...
int ImgDataTEST::T_cpConstructor() {
    ...
    logger->trace(" Constructing new ImgData obj with arg \"data\" and Mat obj...");
    ImgData* data = new ImgData("data",img);
    logger->trace(" Setting Msg to \"Sub rox\"...");
    data->setMsg("Sub rox");
...
//	data->setImg(&img);
    ...
    if (copy->getMsg() == "whatever" && data->getMsg() == "Sub rox")
        logger->trace("    ok");
	else {
    ...
    logger->trace("    \"data\" img refcount: "+std::to_string(datarc));
    logger->trace("    \"copy\" img refcount: "+std::to_string(copyrc));
